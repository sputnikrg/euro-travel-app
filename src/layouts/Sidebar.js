...
export const Sidebar = ({ trips, onAddTrip, fetchTrips, loading, onSelect, selectedTrip }) => {
  ...
  // Удаление рейса (только для перевозчика)
  const handleDeleteTrip = async (e, tripId) => {
    e.stopPropagation();
    if (!window.confirm(t('actions.confirm_delete') || "Удалить этот рейс?")) return;

    const { error } = await supabase.from('trips').delete().eq('id', tripId);
    if (error) alert(error.message);
    else fetchTrips(user.id); // Обновляем список "Моих рейсов"
  };
  ...
  return (
    <div className="sidebar" style={styles.sidebar}>
      ...
      <div style={styles.list}>
        {loading ? <p style={{textAlign:'center'}}>{t('common.loading')}</p> : trips.map(trip => {
          ...
          return (
            <div 
              ...
              style={{
                ...
                backgroundColor: selectedTrip?.id === trip.id ? '#f0f7ff' : '#fff'
              }}
            >
...
const styles = {
  ...
  userBox: { marginBottom: '20px', padding: '10px', backgroundColor: '#fff', borderRadius: '8px', border: '1px solid #eee', fontSize: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  filterButtons: { display: 'flex', gap: '10px', marginBottom: '15px' },
  filterBtn: { flex: 1, padding: '8px', cursor: 'pointer', borderRadius: '6px', border: '1px solid #ddd', backgroundColor: '#fff', fontSize: '11px' },
  ...
  card: { padding: '15px', marginBottom: '10px', borderRadius: '8px', border: '1px solid #eee', cursor: 'pointer' },
  tripDetails: { fontSize: '12px', color: '#666', marginBottom: '10px' },
  cardFooter: { display: 'flex', justifyContent: 'space-between', alignItems: 'center' },
  price: { fontSize: '16px', color: '#2ecc71', fontWeight: 'bold' },
  bookBtn: { padding: '6px 12px', background: '#f39c12', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold' },
